...
function pm_get_fonts_array()
{

    global $pm_fonts;

    return $pm_fonts['items'];

}

function pm_get_fonts_array_only_key_name()
{

    global $pm_fonts;
    $i = 0;

    if (is_array($pm_fonts['items'])) {
        foreach ($pm_fonts['items'] as $key => $font) {
...
function pm_get_theme_fonts()
{
    $protocol = is_ssl() ? 'https' : 'http';

    wp_enqueue_style("AllFonts", "$protocol://fonts.googleapis.com/css?family=" . pm_get_option('content_font_family') . ":700,400,300");
}

add_action('wp_enqueue_scripts', 'pm_get_theme_fonts');