...
/* ========== PUBLIC METHODS ========== */
    ...
    function handle_request () {
		...
		if( ! $validation_mode )
		{
            ...
            $timecheck = time()-3600;
            $mysql->query("DELETE FROM {$prefix}_captcha WHERE time<'".$timecheck."'");
		}
    ...
    function validate ( $user_string) {
        ...
        if( ! $validation_mode )
        {
            $get_data = $mysql->query("SELECT * FROM {$prefix}_captcha WHERE ip='".$_SERVER['REMOTE_ADDR']."'");
            $out_data = $mysql->fetchObject($get_data);


            if ( $md5 == $out_data->code) {
            	$mysql->query("DELETE FROM {$prefix}_captcha WHERE ip='".$_SERVER['REMOTE_ADDR']."'");
                return true;
            }
		}
		else
...
/*
$filename = 'test.txt';
$somecontent = "test: ".$this->image.", $size, $angle, $x, $y, $color, $font,". $this->letters[$i];
...
*/
            ...
            imagettftext ( $this->image, $size, $angle, $x, $y, $color, $font.".ttf", $this->letters[$i]);
        }
    }
	...
	# new function #
    function _put_md5_into_database () {
    	global $prefix, $mysql;
        ...
        $md5                 = md5 ( $string);
        $mysql->query("REPLACE INTO {$prefix}_captcha (id, ip, code, time) VALUES (1, '".$_SERVER['REMOTE_ADDR']."', '".$md5."', '".time()."')");
    }
