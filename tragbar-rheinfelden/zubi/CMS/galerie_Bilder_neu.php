				...
				<tr height="*">
					...
					<td colspan="2">
						...
						<tr>
							<td>Album:</td>
							<td>
								<select name="BildAlbum" id="BildAlbum">
								<?php
									$AlbumAuswahl = mysqli_query($db, "SELECT * FROM bilderalbum WHERE AlbumStatus='1'");
									if($AlbumAuswahl !== 0)
									{
										while ($AlbumAuswahlRow = mysqli_fetch_array($AlbumAuswahl))
										{
											$AlbumID = $AlbumAuswahlRow['ID'];
											$AlbumName = $AlbumAuswahlRow['AlbumName'];
