...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $error_msg .= '<p class="error">Die eingegebene E-Mail Adresse ist ung&uuml;ltig</p>';
    }
    ...
    $prep_stmt = "SELECT id FROM CMSmembers WHERE username = ? LIMIT 1";
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            $error_msg .= '<p class="error">Es existiert bereits ein Benutzer mit diesem Benutzernamen.</p>';
        }
    } else {
        $error_msg .= '<p class="error">Verbindung mit der Datenbank fehlgeschlagen. Bitte versuchen Sie es sp&auml;ter nochmals oder wenden Sie sich an den Systemadministrator.</p>';
    }
	
	$prep_stmt = "SELECT id FROM CMSmembers WHERE email = ? LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows == 1) {
            // Ein Benutzer mit dieser E-Mail-Adresse existiert schon
            $error_msg .= '<p class="error">Es existiert bereits ein Benutzer mit dieser E-Mail-Adresse.</p>';
        }
    } else {
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO CMSmembers (username, email, password, salt) VALUES (?, ?, ?, ?)")) {
